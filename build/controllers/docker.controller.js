...
const startContainer = (params) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const existingContainer = containers.find((container) => container.Names[0] === `/${containerName}`);
        if (existingContainer) {
            const container = docker.getContainer(existingContainer.Id);
            console.log(existingContainer.State);
            if (existingContainer.State !== "running") {
                yield container.remove();
                console.log("Removed existing stopped container:", existingContainer.Id);
            }
            else {
                console.log("Container with the same name is already running:", existingContainer.Id);
                return null;
            }
        }
